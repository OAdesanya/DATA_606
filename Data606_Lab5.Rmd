---
title: "DATA606_Lab5_Inference for numerical data"
author: "Femi Adesanya"
date: "March 25, 2018"
output: 
  html_document: 
    df_print: kable
    fig_caption: yes
    theme: united
    toc: yes
    toc_depth: 6
---

## North Carolina births

In 2004, the state of North Carolina released a large data set containing 
information on births recorded in this state. This data set is useful to 
researchers studying the relation between habits and practices of expectant 
mothers and the birth of their children. We will work with a random sample of 
observations from this data set.

## Exploratory analysis

Load the `nc` data set into our workspace.
```{r}
setwd("C:/Users/26291/Documents/Data606")
```

```{r load-data, eval=TRUE}
download.file("http://www.openintro.org/stat/data/nc.RData", destfile = "nc.RData")
load("nc.RData")
```

```{r}
View(nc)
```


1.  What are the cases in this data set? How many cases are there in our sample?

As a first step in the analysis, we should consider summaries of the data. This
can be done using the `summary` command:

```{r summary, eval=TRUE}
summary(nc)
```

As you review the variable summaries, consider which variables are categorical 
and which are numerical. For numerical variables, are there outliers? If you 
aren't sure or want to take a closer look at the data, make a graph.

Consider the possible relationship between a mother's smoking habit and the 
weight of her baby. Plotting the data is a useful first step because it helps 
us quickly visualize trends, identify strong associations, and develop research
questions.

2.  Make a side-by-side boxplot of `habit` and `weight`. What does the plot 
highlight about the relationship between these two variables?

The box plots show how the medians of the two distributions compare, but we can
also compare the means of the distributions using the following function to 
split the `weight` variable into the `habit` groups, then take the mean of each
using the `mean` function.

```{r by-means, eval=TRUE}
by(nc$weight, nc$habit, mean)
```

There is an observed difference, but is this difference statistically 
significant? In order to answer this question we will conduct a hypothesis test
.

## Inference

3.  Check if the conditions necessary for inference are satisfied. Note that 
you will need to obtain sample sizes to check the conditions. You can compute 
the group size using the same `by` command above but replacing `mean` with 
`length`.
```{r}
by(nc$weight, nc$habit, length)
```

4.  Write the hypotheses for testing if the average weights of babies born to 
smoking and non-smoking mothers are different.
Null Hypothesis : Average birthweight for newborns is independent from a mothers smoking habits
Alternative Hypothesis : Average birthweight for newborns is dependent from a mothers smoking habits

Next, we introduce a new function, `inference`, that we will use for conducting
hypothesis tests and constructing confidence intervals. 

```{r inf-weight-habit-ht, eval=TRUE, tidy=FALSE}
inference(y = nc$weight, x = nc$habit, est = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
```

Let's pause for a moment to go through the arguments of this custom function. 
The first argument is `y`, which is the response variable that we are 
interested in: `nc$weight`. The second argument is the explanatory variable, 
`x`, which is the variable that splits the data into two groups, smokers and 
non-smokers: `nc$habit`. The third argument, `est`, is the parameter we're 
interested in: `"mean"` (other options are `"median"`, or `"proportion"`.) Next
we decide on the `type` of inference we want: a hypothesis test (`"ht"`) or a 
confidence interval (`"ci"`). When performing a hypothesis test, we also need 
to supply the `null` value, which in this case is `0`, since the null 
hypothesis sets the two population means equal to each other. The `alternative`
hypothesis can be `"less"`, `"greater"`, or `"twosided"`. Lastly, the `method` 
of inference can be `"theoretical"` or `"simulation"` based.

5.  Change the `type` argument to `"ci"` to construct and record a confidence 
interval for the difference between the weights of babies born to smoking and 
non-smoking mothers.

By default the function reports an interval for ($\mu_{nonsmoker} - \mu_{smoker}$)
. We can easily change this order by using the `order` argument:

```{r inf-weight-habit-ci, eval=TRUE, tidy=FALSE}
inference(y = nc$weight, x = nc$habit, est = "mean", type = "ci", null = 0, 
          alternative = "twosided", method = "theoretical", 
          order = c("smoker","nonsmoker"))
```

* * *

## On your own

##### 1   Calculate a 95% confidence interval for the average length of pregnancies (`weeks`) and interpret it in context. Note that since you're doing inference on a single population parameter, there is no explanatory variable, so you can omit the `x` variable from the function.

```{r}
inference(y=nc$weeks, est="mean",type="ci",null=0,alternative="twosided",method="theoretical")
```

##### 2.  Calculate a new confidence interval for the same parameter at the 90% confidence level. You can change the confidence level by adding a new argument to the function: `conflevel = 0.90`.

```{r}
inference(y=nc$weeks, est="mean", type="ci", null=0, alternative="twosided", method="theoretical", conflevel=.90)
```

##### 3.  Conduct a hypothesis test evaluating whether the average weight gained by younger mothers is different than the average weight gained by mature mothers.



```{r}
#Null Hypothesis: Average mother's weight gain after birth is independent of age 
#Alternative Hypothesis: Average mother's weight gain after birth is depedent of age

age = ifelse(nc$mage<=mean(nc$mage),"Younger",'Older')
inference(y=nc$weeks, x=age, est="median", type="ht", null=0, alternative="twosided", method="simulation")

# With a P value of  we can reject the null hypothesis and accept the alternative hypothesis that average mother's weight gain after birth is dependent of age.
```



##### 4.Now, a non-inference task: Determine the age cutoff for younger and mature mothers. Use a method of your choice, and explain how your method works.

###### Ans: I will use a subset between younger and older mother to calculate the minimum and maximum age.

```{r}
#using a subset

#younger mother age range
c(min(subset(nc, mature == 'younger mom')$mage),max(subset(nc, mature == 'younger mom')$mage))
```

```{r}
#mature mother age range
c(min(subset(nc, mature != 'younger mom')$mage),max(subset(nc, mature != 'younger mom')$mage))
```



##### 5.Pick a pair of numerical and categorical variables and come up with a 
research question evaluating the relationship between these variables. 
Formulate the question in a way that it can be answered using a hypothesis test
and/or a confidence interval. Answer your question using the `inference` 
function, report the statistical results, and also provide an explanation in 
plain language.

Null Hypothesis: Average weight of a mother is independent of marital status
Alternative Hypothesis: Average weight of a mother depends on marital status

```{r}
inference(y=nc$weight,x=nc$marital,est='mean',type='ht',null = 0,alternative = "twosided",method = 'theoretical')
```

###### In this test, we examined whether or not the average weight of a mother was dependent on marital status. Our test revealed that
the chances of it being independent of marital status was 0. Therefore we will reject our null hypothesis and acceppt our alternativehypothesis that the average weight of a mother depends on her marital status.


